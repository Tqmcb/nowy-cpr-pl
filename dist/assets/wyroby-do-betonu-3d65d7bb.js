const n=`---
title: "Wyroby do betonu i zapraw – wymagania CPR 2024"
family_number: 26
family_name_en: "PRODUCTS RELATED TO CONCRETE, MORTAR AND GROUT"
date: "2026-02-25"
category: "Chemia budowlana"
avs_system: "2+ / 3"
normy: ["EN 934-2", "EN 934-3", "EN 934-4", "EN 934-5", "EN 480 (seria)", "EN 450-1", "EN 15167-1", "EN 13263-1"]
tags: ["beton", "domieszki", "plastyfikatory", "CPR 2024", "EN 934"]
excerpt: "Wyroby do betonu i zapraw – rodzina 26 CPR 2024. Domieszki plastyfikujące, napowietrzające, przyspieszające, opóźniające. EN 934 seria."
image_url: "https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=800"
---

# Wyroby do betonu, zapraw i zastrzyków – rodzina wyrobów 26 wg CPR 2024/3110

Domieszki do betonu i zapraw to jedna z najbardziej dynamicznie rozwijających się kategorii wyrobów budowlanych, której znaczenie systematycznie rośnie wraz z wymaganiami dotyczącymi trwałości i efektywności środowiskowej konstrukcji betonowych. Rozporządzenie CPR 2024/3110 obejmuje tę rodzinę numerem 26 w Załączniku VII i kładzie szczególny nacisk na alternatywne spoiwa o obniżonym śladzie węglowym.

## O rodzinie wyrobów

Rodzina 26 obejmuje wyroby stosowane w technologii betonów i zapraw jako składniki modyfikujące właściwości świeżej lub stwardniałej mieszanki. Dzielą się na dwie główne podgrupy:

**Domieszki chemiczne** – dodawane w ilości ≤ 5% masy cementu, wywierające efekt chemiczny lub fizyczny: domieszki uplastyczniające i upłynniające (superplastyfikatory), napowietrzające, przyspieszające wiązanie i twardnienie, opóźniające wiązanie, uszczelniające, do betonu natryskowego (shotcrete).

**Dodatki mineralne (additions)** – dodawane w ilości > 5% masy cementu jako częściowy zamiennik klinkieru: popiół lotny (fly ash), granulowany żużel wielkopiecowy (GGBS), pył krzemionkowy (mikrosilika). To właśnie ta podgrupa zyskuje największe znaczenie w CPR 2024 ze względu na potencjał redukcji emisji CO2.

Do rodziny 26 należą również wyroby do naprawy betonu (zaprawy naprawcze wg EN 1504) i zaczynu iniekcyjnego.

## Normy zharmonizowane

| Wyrób | Norma zharmonizowana |
|---|---|
| Domieszki do betonu | EN 934-2:2009+A1:2012 |
| Domieszki do zapraw murarskich | EN 934-3:2009+A1:2012 |
| Domieszki do zaczynu iniekcyjnego dla kabli sprężających | EN 934-4:2009 |
| Domieszki do betonu natryskowego | EN 934-5:2007+A1:2012 |
| Metody badań domieszek | EN 480 (seria) |
| Popiół lotny do betonu | EN 450-1:2012 |
| Granulowany żużel wielkopiecowy (GGBS) | EN 15167-1:2006 |
| Pył krzemionkowy (mikrosilika) | EN 13263-1:2005+A1:2009 |

## System AVS i certyfikacja

Dla rodziny 26 obowiązują systemy AVS (Assessment and Verification of Constancy of Performance) nr **2+** i **3**:

- **System AVS 2+** – dla domieszek do betonu (EN 934-2) i kluczowych dodatków mineralnych: jednostka notyfikowana przeprowadza certyfikację zakładowej kontroli produkcji (ZKP), a producent wykonuje wstępne badanie typu. Certyfikat ZKP jest wymagany – bez niego nie można wystawić DoP&C.
- **System AVS 3** – dla domieszek do zapraw murarskich (EN 934-3) i betonu natryskowego (EN 934-5): jednostka notyfikowana przeprowadza wstępne badanie typu (ITT), producent samodzielnie certyfikuje ZKP.

W systemie AVS 2+ producent wystawia **Deklarację Właściwości Użytkowych i Zgodności (DoP&C)**, a certyfikat ZKP wystawiony przez jednostkę notyfikowaną stanowi jej obowiązkowy załącznik. Dla deklaracji środowiskowych (EPD) zastosowanie ma nowy **system 3+** – walidacja przez jednostkę notyfikowaną.

## Kluczowe wymagania CPR 2024

**Właściwości użytkowe domieszek chemicznych (EN 934-2):**
- Uplastycznienie i upłynnienie: redukcja wody (W/C) o ≥ 5% (plastyfikatory) lub ≥ 12% (superplastyfikatory) przy zachowaniu konsystencji
- Wciąganie powietrza: zawartość powietrza w betonie i jego stabilność (napowietrzające)
- Przyspieszanie: wytrzymałość po 24h ≥ 120% próbki kontrolnej
- Opóźnianie: czas wiązania wydłużony o ≥ 90 minut

**Właściwości środowiskowe – priorytet CPR 2024:**
- Potencjał globalnego ocieplenia (GWP) – deklaracja obowiązkowa dla dodatków mineralnych
- Zawartość substancji regulowanych (chlorki, alkalia) – limity wpływające na trwałość zbrojenia
- Biodegradowalność składników organicznych domieszek

**BWR 7 i niskoemisyjne spoiwa:**
CPR 2024 po raz pierwszy wyraźnie stwarza ramy dla **uzupełniających materiałów cementowych (SCM)** – popiołów lotnych, GGBS i mikrosiliki – jako instrumentów redukcji emisji CO2 w budownictwie (do 40–60% redukcji GWP vs. beton ze 100% klinkieru).

## Zmiany względem CPR 305/2011

1. **Wzmocnienie BWR 7** – nowe wymagania środowiskowe dla dodatków mineralnych jako niskoemisyjnych zamienników klinkieru.
2. **Deklaracja emisji i toksyczności** – producenci domieszek chemicznych muszą deklarować zawartość SVHC i potencjalne uwalnianie substancji niebezpiecznych do betonu i środowiska.
3. **System 3+ dla EPD** – jednostka notyfikowana waliduje deklaracje środowiskowe dla domieszek i dodatków.
4. **Uproszczony nadzór rynkowy** – wzmocnione uprawnienia organów nadzoru do pobierania próbek w terenie i weryfikacji DoP&C.

## Harmonogram wdrożenia

| Etap | Data |
|---|---|
| Wejście w życie CPR 2024/3110 | 8 stycznia 2025 |
| Pełne stosowanie przepisów | **8 stycznia 2026** |
| Obowiązek stosowania kar przez państwa członkowskie | **8 stycznia 2027** |

## Przykłady wyrobów i zastosowania

**Superplastyfikatory polikarboksylanowe (PCE)** – redukcja W/C o 20–40%; betony wysokowartościowe (HPC) i samozagęszczalne (SCC); mosty, tunele, wieżowce.

**Domieszki napowietrzające** – mrozoodporność betonu dróg i chodników; lotniska, drogi ekspresowe, place przemysłowe.

**Domieszki przyspieszające do shotcrete** – budowa tuneli metodą NATM; wstępna wytrzymałość po 1h ≥ 2 MPa.

**Popiół lotny EN 450-1 (klasa S i N)** – zamiennik cementu do 35% w masie; obniżenie ciepła hydratacji w fundamentach masywnych; redukcja GWP o 20–30%.

**GGBS EN 15167-1** – zamiennik cementu do 80% w masie; wysokoodporne betony morskie i narażone na siarczany.

## Checklist producenta

- [ ] Zidentyfikować właściwą normę z serii EN 934 lub EN 450/EN 15167/EN 13263
- [ ] Wybrać system AVS (2+ lub 3) i zlecić audyt ZKP lub ITT jednostce notyfikowanej
- [ ] Uzyskać certyfikat ZKP od jednostki notyfikowanej (system AVS 2+)
- [ ] Przeprowadzić badania chlorków, alkaliów i substancji szkodliwych
- [ ] Sporządzić DoP&C w nowym formacie CPR 2024 i umieścić oznakowanie CE
- [ ] Ocenić i zadeklarować GWP produktu (BWR 7)
- [ ] Sprawdzić zawartość substancji SVHC i zarejestrować w bazie SCIP
- [ ] Zaktualizować kartę charakterystyki (SDS) zgodnie z CLP i REACH
- [ ] Rozważyć opracowanie EPD i jej walidację w systemie 3+

## Gdzie szukać wsparcia

Informacje o certyfikacji domieszek do betonu i wymaganiach CPR 2024 dostępne są na [nowycpr.pl](https://www.nowycpr.pl). Europejskie wytyczne techniczne dla domieszek publikuje [EFCA (European Federation of Concrete Admixtures Associations)](https://www.efca.info). Normy EN 934 dostępne są przez [PKN](https://www.pkn.pl).
`;export{n as default};
