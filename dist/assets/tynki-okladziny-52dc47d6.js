const n=`---
title: "Tynki, okładziny i ścianki działowe – wymagania CPR 2024"
family_number: 21
family_name_en: "INTERNAL AND EXTERNAL WALL AND CEILING FINISHES. INTERNAL PARTITION KITS"
date: "2026-02-25"
category: "Wyroby wykończeniowe"
avs_system: "3 / 4"
normy: ["EN 998-1", "EN 15824", "EN 15825", "EN 13963", "EN 13914-1"]
tags: ["tynki", "okładziny", "ściany", "gips", "CPR 2024"]
excerpt: "Tynki, okładziny i ścianki działowe – rodzina 21 CPR 2024. Tynki cementowe, gipsowe, akrylowe, silikonowe; ścianki GK. Kluczowe zmiany CPR 2024: wymagania VOC dla wyrobów wewnętrznych i deklarowanie GWP."
image_url: "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=800"
---

## O rodzinie wyrobów

Rodzina 21 w Załączniku VII CPR 2024/3110 obejmuje wykończenia wewnętrznych i zewnętrznych powierzchni ścian i sufitów oraz zestawy ścianek działowych. Jest to rodzina obejmująca ogromną różnorodność produktów: od tradycyjnych tynków cementowo-wapiennych przez nowoczesne tynki silikonowe i silikatowe, po systemy suchej zabudowy z płyt gipsowo-kartonowych (GK) i gipsowo-włóknowych (GF).

Z perspektywy CPR 2024/3110 rodzina 21 jest szczególnie istotna ze względu na wymaganie podstawowe nr 3 (Higiena, zdrowie i środowisko), gdyż tynki i okładziny wewnętrzne są w bezpośrednim kontakcie z powietrzem w pomieszczeniach i mogą być źródłem emisji VOC. Nowe rozporządzenie formalizuje obowiązek deklarowania emisji dla wyrobów do zastosowań wewnętrznych.

Rodzina 21 obejmuje również **systemy ETICS** (ang. External Thermal Insulation Composite System – złożone systemy ociepleń elewacji), które jednak ze względu na złożoność najczęściej certyfikowane są poprzez ETA, a nie normy zharmonizowane.

## Normy zharmonizowane

| Norma | Zakres |
|---|---|
| EN 998-1:2016 | Zaprawy tynkarskie (rendering/plastering mortar) |
| EN 998-2:2016 | Zaprawy murarskie (masonry mortar) |
| EN 15824:2009 | Wyroby organiczne do tynkowania zewnętrznego i wewnętrznego |
| EN 15825:2010 | Farby i powłoki do tynkowania zewnętrznego i wewnętrznego |
| EN 13963:2014 | Materiały do spoinowania płyt gipsowych |
| EN 13914-1:2005 | Projektowanie, przygotowanie i aplikacja tynków zewnętrznych |
| EN 13914-2:2005 | Projektowanie, przygotowanie i aplikacja tynków wewnętrznych |
| EN 520:2004+A1:2009 | Płyty gipsowo-kartonowe – definicje, wymagania i metody badań |
| EN 15283-1:2008 | Płyty gipsowo-włóknowe – wymagania, definicje |
| EN 14190:2014 | Wtórnie przetworzone wyroby gipsowe |

## System AVS i certyfikacja

Dla wyrobów z rodziny 21 obowiązuje:

- **System AVS 3** – dla tynków zewnętrznych stosowanych w obiektach o wymaganiach ogniowych lub konstrukcyjnych, oraz dla zestawów ścianek działowych o deklarowanej odporności ogniowej (EI 30, EI 60, EI 120). Jednostka notyfikowana przeprowadza badania ITT (w tym badania ogniowe).
- **System AVS 4** – dla większości tynków dekoracyjnych, materiałów do spoinowania i wyrobów pomocniczych. Producent samodzielnie przeprowadza badania i deklaruje właściwości.
- **System AVS 3+** – walidacja EPD dla deklaracji GWP i wskaźników środowiskowych.

Badania tynków i okładzin (emisje VOC, reakcja na ogień, wodoszczelność) wykonuje m.in. [ITCZLin](https://www.itczlin.pl). Certyfikację oferuje [Multicert](https://www.multicert.pl).

## Kluczowe wymagania CPR 2024

**Klasyfikacja tynków (EN 998-1):**
- **CS I** – Wytrzymałość na ściskanie 0,4–2,5 N/mm² (tynki lekkie, izolacyjne)
- **CS II** – 1,5–5,0 N/mm² (tynki normalne, standard)
- **CS III** – 3,5–7,5 N/mm² (tynki wzmocnione)
- **CS IV** – ≥6 N/mm² (tynki techniczne, cokołowe)

Dodatkowe cechy charakterystyczne tynków EN 998-1:
- **Absorpcja wody przez kapilarność** (klasa W0, W1, W2) – dla elewacji
- **Przepuszczalność pary wodnej** (klasa V1, V2, V3) – szczególnie ważna dla budownictwa pasywnego
- **Przyczepność do podłoża** (min. 0,08 N/mm² dla tynków zwykłych)
- **Odporność ogniowa** (w zależności od kompozycji)

**Tynki organiczne (EN 15824) – kategorie:**
- Tynki akrylowe (żywice akrylowe w dyspersji wodnej)
- Tynki silikonowe (polimery silikonowe – hydrofobowe, paroprzepuszczalne)
- Tynki silikatowe (szkło wodne – mineralne, paroprzepuszczalne)
- Tynki mozaikowe i kruszcowe (ziarniste, do cokołów)

**Wymagania VOC (kluczowe w CPR 2024):**

CPR 2024/3110 rozszerza wymaganie podstawowe nr 3 o obowiązek deklarowania emisji VOC dla tynków wewnętrznych. Limity harmonizują się z dyrektywą 2004/42/CE (dyrektywa o rozpuszczalnikach w farbach):
- Tynki wewnętrzne wodne: max TVOC 30–75 g/l (zależnie od zastosowania)
- Tynki wewnętrzne solwentowe: max TVOC 150–200 g/l
- Tynki „zero-VOC": <5 g/l (rynek premium, etykiety ekologiczne)

**Odporność ogniowa ścianek działowych:**

Systemy GK (płyty gipsowo-kartonowe) są najczęściej testowane jako zestawy (kombinacja płyt, profili, wełny mineralnej, masy spoinowej). Typowe klasyfikacje:
- Ścianka jednowarstwowa 1×12,5 GK na profilach 75 mm: EI 30 A2-s1,d0
- Ścianka dwuwarstwowa 2×12,5 GK na profilach 100 mm z wełną: EI 60–EI 120

**Wymagania środowiskowe (CPR 2024):**
- Tynki gipsowe: GWP ~0,15–0,25 kg CO2eq/kg (niskie, gips jest surowcem o małej energochłonności produkcji)
- Tynki cementowe: GWP ~0,2–0,4 kg CO2eq/kg (zawartość cementu determinuje GWP)
- Tynki akrylowe: GWP ~0,8–1,5 kg CO2eq/kg (petrochemiczne spoiwo)

## Zmiany względem CPR 305/2011

1. **Obowiązkowe deklarowanie emisji VOC** dla tynków i okładzin wewnętrznych – formalizacja wymagania dotąd stosowanego dobrowolnie.
2. **Deklarowanie GWP w DoP&C** – nowe wymaganie dla całej rodziny.
3. **Substancje SVHC** – producenci tynków organicznych muszą sprawdzić stosowane rozpuszczalniki i środki biobójcze (biocydy elewacyjne) pod kątem REACH.
4. **Systemy ETICS** – EAD wydane pod CPR 305/2011 tracą ważność 9 stycznia 2031; nowe systemy ETICS wymagają ETA na podstawie nowych EAD.
5. **DoP&C** – rozszerzona deklaracja łącząca właściwości użytkowe i środowiskowe.

## Harmonogram wdrożenia

| Data | Wydarzenie |
|---|---|
| 8 stycznia 2026 | Pełne stosowanie CPR 2024/3110 – DoP&C z deklaracją VOC |
| 8 stycznia 2027 | Wejście w życie sankcji |
| 9 stycznia 2031 | Wygaśnięcie EAD dla systemów ETICS z CPR 305/2011 |
| 7 stycznia 2040 | Koniec okresu przejściowego |

## Przykłady wyrobów

**Przykład 1: Producent tynków silikonowych do elewacji**

Producent tynku silikonowego ziarnistego (baranek 1,5 mm) przeznaczonego do wykończeń elewacyjnych:
1. Badania ITT wg EN 15824: wytrzymałość na ściskanie, przyczepność, przepuszczalność pary, absorpcja wody, mrozoodporność, trwałość powłoki
2. System AVS 4 – producent przeprowadza badania samodzielnie w akredytowanym laboratorium
3. Deklaracja zawartości VOC: tynk silikonowy na bazie wodnej – TVOC 8 g/l (poniżej progu dyrektywy 42/CE)
4. Sprawdzenie biocydów stosowanych do ochrony przed algami i grzybami (REACH/BPR)
5. DoP&C: CS II, W1, V2, klasa A1 (niepalny), TVOC 8 g/l, GWP 1,1 kg CO2eq/kg

**Przykład 2: Producent systemu ścianek GK z deklaracją EI 60**

Producent systemu ścianek działowych (profile stalowe + płyty GK + masa spoinowa):
1. Badania ogniowe zestawu wg EN 1364-1 w akredytowanym laboratorium – uzyskany wynik EI 60
2. System AVS 3 – NB przeprowadza ITT, producent prowadzi ZKP
3. Klasyfikacja wg EN 13501-2: EI 60
4. DoP&C dla zestawu: EI 60, odporność akustyczna Rw=45dB, A2-s1,d0

## Checklist producenta

- [ ] Identyfikacja normy (EN 998-1, EN 15824, EN 15825, EN 13963, EN 520 lub inne)
- [ ] Ustalenie systemu AVS (3 dla wyrobów z deklarowaną odpornością ogniową lub zewnętrznych, 4 dla pozostałych)
- [ ] Przeprowadzenie badań ITT w akredytowanym laboratorium ([ITCZLin](https://www.itczlin.pl))
- [ ] Badania emisji VOC i substancji niebezpiecznych dla tynków wewnętrznych
- [ ] Sprawdzenie biocydów stosowanych w tynkach elewacyjnych (Rozporządzenie BPR)
- [ ] Wdrożenie ZKP z monitoringiem składu i właściwości
- [ ] Wystawienie DoP&C z deklaracją klasy CS, kategorii W, V, VOC, GWP
- [ ] Dla systemów ETICS: uzyskanie lub aktualizacja ETA przez [ocenytechniczne.pl](https://www.ocenytechniczne.pl)
- [ ] Oznakowanie CE na opakowaniu lub dokumentach dostawy
- [ ] Przegląd daty ważności EAD dla systemów ETICS (9 stycznia 2031)

## Gdzie szukać wsparcia

- [Multicert](https://www.multicert.pl) – certyfikacja wyrobów wykończeniowych i tynkarskich
- [ITCZLin](https://www.itczlin.pl) – badania emisji VOC, właściwości mechanicznych tynków i zapraw, badania ogniowe
- [ocenytechniczne.pl](https://www.ocenytechniczne.pl) – ETA dla systemów ETICS i innowacyjnych systemów suchej zabudowy
`;export{n as default};
