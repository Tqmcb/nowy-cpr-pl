---
title: "Uszczelnienia szczelin – wymagania CPR 2024"
family_number: 32
family_name_en: "SEALANTS FOR JOINTS"
date: "2026-02-25"
category: "Chemia budowlana"
avs_system: "3 / 4 / 3+"
normy: ["EN 15651-1", "EN 15651-2", "EN 15651-3", "EN 15651-4", "EN 15651-5"]
tags: ["uszczelniacze", "silikon", "poliuretan", "fasada", "szczeliny dylatacyjne", "CPR 2024"]
excerpt: "Uszczelnienia szczelin – rodzina 32 CPR 2024. Uszczelniacze fasadowe, sanitarne, do szyb, drogowe. Odkształcalność i trwałość."
image_url: "https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=800"
---

# Uszczelnienia szczelin – rodzina wyrobów 32 wg CPR 2024/3110

Uszczelniacze szczelin budowlanych to wyroby chemiczne stosowane do wypełniania i uszczelniania połączeń między elementami budowlanymi, które pozostają ruchome w trakcie eksploatacji. Rozporządzenie CPR 2024/3110 obejmuje tę rodzinę numerem 32 w Załączniku VII i szczególnie akcentuje wymagania dotyczące emisji VOC dla uszczelniaczy stosowanych wewnątrz pomieszczeń.

## O rodzinie wyrobów

Rodzina 32 – Sealants for Joints – obejmuje gotowe do użycia wyroby przeznaczone do uszczelniania szczelin, dylatacji i połączeń w elementach i obiektach budowlanych. Klasyfikacja według serii EN 15651 dzieli uszczelniacze na pięć grup zastosowań:

- **Fasadowe (F)** – uszczelnienia zewnętrzne: połączenia między elementami prefabrykowanymi, panelami elewacyjnymi, ościeżnicami okiennymi i ścianą
- **Do szklenia (G – Glazing)** – uszczelniacze do osadzania szyb w ramach okiennych i fasadach szklanych
- **Sanitarne (S)** – do stosowania w kuchniach, łazienkach, obiektach basenowych; wymagana odporność na grzyby i wilgoć
- **Basenowe (P – Swimming pool)** – trwałe zanurzenie w wodzie chlorowanej
- **Drogowe (R – Road)** – szczeliny w nawierzchniach dróg, lotnisk, placów; odporność na paliwa i duże odkształcenia termiczne

Chemicznie uszczelniacze dzielą się na: **silikonowe** (najwyższa trwałość UV i termiczna), **poliuretanowe** (malowalne, dobre przyczepność do betonu), **polimerowe MS (Modified Silicone)** i **polimerowe epoksydowe** (specjalne zastosowania).

## Normy zharmonizowane

| Wyrób | Norma zharmonizowana |
|---|---|
| Uszczelniacze fasadowe (F) | EN 15651-1:2012 |
| Uszczelniacze do szklenia i do stosowania w budownictwie ogólnym | EN 15651-2:2012 |
| Uszczelniacze sanitarne (S) | EN 15651-3:2012 |
| Uszczelniacze drogowe (R) | EN 15651-4:2012 |
| Uszczelniacze basenowe (P) | EN 15651-5:2017 |

## System AVS i certyfikacja

Dla rodziny 32 obowiązują systemy AVS (Assessment and Verification of Constancy of Performance) nr **3** i **4**:

- **System AVS 4** – dla uszczelniaczy sanitarnych klasy 12,5 w zastosowaniach wewnętrznych standardowych: producent samodzielnie wykonuje badania wstępne i prowadzi ZKP.
- **System AVS 3** – dla uszczelniaczy fasadowych, szklarskich i drogowych, a także uszczelniaczy klasy 25 i wyżej: notyfikowane laboratorium techniczne (NTL) przeprowadza wstępne badanie typu (ITT), a jednostka notyfikowana (JN) wydaje **certyfikat właściwości użytkowych i zgodności wyrobu**. Producent wystawia DoP&C na podstawie certyfikatu JN; numer JN pojawia się w oznakowaniu CE.

**System AVS 3+** – walidacja danych środowiskowych. Gdy producent deklaruje w DoP&C właściwości środowiskowe (np. GWP — potencjał globalnego ocieplenia, obliczony metodą LCA zgodnie z EN 15804+A2), wyniki te muszą zostać zwalidowane przez jednostkę notyfikowaną w Systemie AVS 3+. Walidacja potwierdza rzetelność obliczeń LCA i ma zastosowanie, gdy producent deklaruje właściwości środowiskowe w DoP&C.

## Kluczowe wymagania CPR 2024

**Odkształcalność (klasa ruchu):**
- **12,5** – zdolność do odkształceń ±12,5% szerokości szczeliny nominalnej (standardowe dylatacje)
- **25** – ±25% szerokości (fasady z dużymi ruchami termicznymi, szczeliny przy płytach balkonowych)
- **12,5E** – ±12,5% dla zastosowań z obciążeniem mimośrodowym

Klasa odkształcalności jest najważniejszym parametrem użytkowym uszczelniaczy i musi być wyraźnie zadeklarowana w DoP&C.

**Przyczepność do podłoży (adhezja):**
- Badanie metodą H lub T wg EN ISO 8339/8340
- Podłoża referencyjne: beton, aluminium, szkło, PVC – przyczepność bez i z gruntem
- Brak kohezyjnego uszkodzenia po rozciąganiu do 100% odkształcenia

**Trwałość i odporność:**
- Odporność na UV i starzenie atmosferyczne (UV/AW) – szczególnie dla klasy F (fasadowe)
- Odporność na temperatury: od -40°C do +150°C (uszczelnienia fasadowe w Polsce: od -30°C do +80°C)
- Odporność na grzyby (dla klasy S i P) wg EN 15651-3 metoda C
- Odporność na chlor i środki czyszczące (klasa P – basenowe)

**Emisja VOC (nowe wymaganie obowiązkowe w CPR 2024):**
- Wszystkie uszczelniacze do zastosowań wewnętrznych (klasy S, G do wnętrz) muszą deklarować emisję VOC po 3 dniach (C3) i 28 dniach (C28)
- Limity wynikające z przepisów krajowych (np. etykiety środowiskowe) lub wymagań zamawiającego

## Zmiany względem CPR 305/2011

1. **Obowiązkowa deklaracja emisji VOC** – wcześniej opcjonalna, CPR 2024 czyni ją obowiązkową dla wszystkich uszczelniaczy stosowanych wewnątrz budynków, co szczególnie dotyczy klas S (sanitarne) i G (szklarskie wewnętrzne).
2. **BWR 7 – właściwości środowiskowe** – producenci uszczelniaczy muszą ocenić GWP wyrobu oraz potencjalną emisję substancji niebezpiecznych (izocyjaniany z PU, cykliczne siloksany z silikonu) w całym cyklu życia.
3. **Rozszerzenie klasy P (basenowe)** – EN 15651-5 jest stosunkowo nową normą (2017) i CPR 2024 pierwszą nowelizacją, w której uszczelniacze basenowe uzyskują pełne umocowanie prawne w systemie CPR.
4. **Nowy format DoP&C** – zastąpienie dotychczasowej DoP nowszym dokumentem z obowiązkową sekcją środowiskową.

## Harmonogram wdrożenia

| Etap | Data |
|---|---|
| Wejście w życie CPR 2024/3110 | 8 stycznia 2025 |
| Pełne stosowanie przepisów | **8 stycznia 2026** |
| Obowiązek stosowania kar przez państwa członkowskie | **8 stycznia 2027** |

## Przykłady wyrobów i zastosowania

**Uszczelniacz silikonowy fasadowy klasa F 25 LM** – niski moduł Young'a (LM – Low Modulus) pozwala na duże odkształcenia przy niskich naprężeniach; doskonała przyczepność do szkła i aluminium bez gruntu; fasady strukturalne i półstrukturalne.

**Uszczelniacz poliuretanowy fasadowy klasa F 25** – malowalny; stosowany do połączeń między elementami prefabrykowanymi betonowymi i murem; szarozielony kolor maskujący po malowaniu elewacji.

**Uszczelniacz silikonowy sanitarny z fungicydami klasa S 12,5** – odporny na grzyby (Candida, Aspergillus); łazienki, kabiny prysznicowe; niska emisja VOC (kategoria A+ wg etykiety NF)

**Uszczelniacz drogowy hot-pour klasa R 25** – bitumiczny lub poliuretanowy; aplikacja w temperaturze 170°C; szczeliny w nawierzchniach betonowych i asfaltobetonowych lotnisk i autostrad.

## Checklist producenta

- [ ] Określić klasę zastosowania (F/G/S/P/R) i właściwą normę z serii EN 15651
- [ ] Określić wymaganą klasę odkształcalności (12,5 / 12,5E / 25) i moduł Young'a
- [ ] Wybrać system AVS (3 lub 4) i zlecić ITT laboratatorium notyfikowanemu (AVS 3)
- [ ] Przetestować przyczepność do podłoży referencyjnych (beton, szkło, aluminium, PVC)
- [ ] Przeprowadzić badania emisji VOC dla zastosowań wewnętrznych
- [ ] Ocenić GWP i zawartość substancji niebezpiecznych (izocyjaniany, siloksany) – BWR 7
- [ ] Wdrożyć ZKP i prowadzić dokumentację kontroli partii produkcyjnych
- [ ] Walidacja danych środowiskowych przez JN (System AVS 3+) dla deklaracji GWP w DoP&C
- [ ] Wystawić DoP&C w formacie CPR 2024 i umieścić oznakowanie CE
- [ ] Zaktualizować SDS (kartę charakterystyki) z informacjami o SVHC

## Gdzie szukać wsparcia

Szczegółowe informacje o certyfikacji uszczelniaczy budowlanych i wymaganiach CPR 2024 dostępne są na [nowycpr.pl](https://www.nowycpr.pl). Badania ITT dla uszczelniaczy przeprowadzają laboratoria notyfikowane, w tym [IFT Rosenheim](https://www.ift-rosenheim.de). Europejskie stowarzyszenie producentów uszczelniaczy i klejów [FEICA](https://www.feica.eu) publikuje wytyczne techniczne i interpretacyjne. Normy EN 15651 dostępne są przez [PKN](https://www.pkn.pl).
