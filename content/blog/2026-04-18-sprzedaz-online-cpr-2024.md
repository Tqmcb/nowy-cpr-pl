---
title: "Sprzedaż wyrobów budowlanych online — obowiązki sprzedawcy w CPR 2024"
date: "2026-04-18"
author: "mec. Paweł Szczepański"
category: "Prawo"
tags: ["sprzedaż online", "e-commerce", "CPR 2024", "dystrybutor", "obowiązki sprzedawcy"]
excerpt: "CPR 2024/3110 po raz pierwszy kompleksowo reguluje sprzedaż wyrobów budowlanych w kanałach online. Dystrybutorzy prowadzący własne sklepy internetowe lub wystawiający produkty na platformach marketplace muszą spełnić nowe obowiązki informacyjne. Omawiamy, co musi zawierać listing produktu, jakie prawa ma nabywca i jak wygląda odpowiedzialność w łańcuchu dostaw."
image_url: "https://images.unsplash.com/photo-1563013544-824ae1b704d3?w=800"
template: "analiza"
---

## Dlaczego e-commerce wyrobów budowlanych wymaga osobnej analizy?

Tradycyjny kanał dystrybucji wyrobów budowlanych — hurtownia, skład materiałów, bezpośrednia sprzedaż producenta — podlegał regulacjom CPR 305/2011, choć przepisy te nie odnosiły się wprost do sprzedaży internetowej. CPR 2024/3110 wypełnia tę lukę, wyraźnie włączając sprzedaż na odległość i sprzedaż przez platformy elektroniczne w zakres obowiązków uczestników łańcucha dostaw.

W niniejszym artykule analizujemy sytuację trzech typów podmiotów działających w e-commerce wyrobów budowlanych:
- producenta prowadzącego własny sklep internetowy (B2B i B2C),
- dystrybutora (sprzedawcy hurtowego lub detalicznego) oferującego wyroby przez własną stronę,
- sprzedawcy oferującego wyroby na platformie marketplace (Allegro, Amazon, Ceneo itp.).

---

## Definicja dystrybutora w CPR 2024 a sklep internetowy

Art. 3 CPR 2024 definiuje **dystrybutora** jako każdą osobę fizyczną lub prawną w łańcuchu dostaw, niebędącą producentem ani importerem, która udostępnia wyrób budowlany na rynku. W kontekście e-commerce oznacza to, że prowadzenie sklepu internetowego z wyrobami budowlanymi jest działalnością dystrybucyjną w pełnym znaczeniu CPR 2024.

Kluczowa zasada: dystrybutor nie jest odpowiedzialny za pierwotną ocenę właściwości wyrobu (to zadanie producenta), ale jest odpowiedzialny za:

1. weryfikację, że wyrób posiada wymagane oznakowanie CE (jeżeli norma zharmonizowana je nakazuje),
2. weryfikację, że wyrobowi towarzyszy prawidłowa i aktualna DoP&C,
3. nieprzedstawianie wyrobu w sposób, który mógłby wprowadzić nabywcę w błąd co do jego właściwości,
4. niezwłoczne powiadomienie producenta lub importera, a w razie potrzeby organów nadzoru rynku, jeśli stwierdzi lub ma podstawy do podejrzewania, że wyrób jest niezgodny.

---

## Wymagania dotyczące listingu produktu w sklepie internetowym

### Obowiązek informacyjny przed zakupem

Art. 7 CPR 2024 w połączeniu z przepisami dyrektywy o prawach konsumentów (2011/83/UE) nakłada na sprzedawcę internetowego obowiązek zapewnienia nabywcy dostępu do DoP&C **przed zawarciem umowy**. W praktyce oznacza to, że każdy listing produktu objętego normą zharmonizowaną powinien zawierać:

- **Widoczne oznakowanie CE** — logo CE lub wyraźna informacja, że wyrób nosi oznakowanie CE wraz z numerem normy zharmonizowanej.
- **Link do aktualnej DoP&C** — hiperłącze do strony producenta lub platformy cyfrowej, gdzie dokument jest bezpłatnie dostępny do pobrania. Brak tego linku w momencie zakupu może stanowić naruszenie obowiązków dystrybutora wynikających z CPR 2024.
- **Podstawowe dane identyfikacyjne wyrobu** — typ wyrobu zgodny z DoP&C, numer normy zharmonizowanej.
- **Zamierzone zastosowanie** — przynajmniej skrótowy opis, dla jakiego zastosowania wyrób jest przeznaczony i w jakich warunkach.

### Zdjęcia i opisy — pułapka dla sprzedawcy

Zarówno CPR 2024, jak i ogólne przepisy o nieuczciwych praktykach handlowych (dyrektywa 2005/29/WE) zakazują prezentowania wyrobów w sposób sugerujący właściwości, których wyrób nie posiada lub które nie zostały zdeklarowane. Sprzedawca, który w opisie produktu podaje klasę reakcji na ogień A1, a w DoP&C producenta zadeklarowana jest klasa E, naraża się na odpowiedzialność za wprowadzenie nabywcy w błąd — niezależnie od winy producenta.

---

## Sprzedaż przez platformy marketplace — Amazon, Allegro i podobne

Sytuacja sprzedawcy na marketplace jest pod wieloma względami trudniejsza niż prowadzenie własnego sklepu, ponieważ:

1. **Platforma marketplace nie jest dystrybutorem** w rozumieniu CPR 2024 — odpowiedzialność spoczywa na sprzedawcy wystawiającym produkt.
2. **Regulaminy platform** (np. Amazon Product Compliance Policy, Allegro regulamin sprzedaży) wymagają zgodności z przepisami unijnymi, w tym z CPR. Wystawienie wyrobu bez wymaganego oznakowania CE może skutkować usunięciem listingu przez platformę.
3. **Pole opisu produktu na marketplace** musi zawierać te same informacje, co listing na własnym sklepie — w szczególności link do DoP&C. Jeżeli platforma nie umożliwia zamieszczenia bezpośredniego linku, należy podać URL w sekcji dokumentów lub specyfikacji technicznej.
4. **Sprzedaż transgraniczna** — wystawiając wyrób na Amazon.de, Amazon.fr czy Allegro z wysyłką do innych krajów UE, sprzedawca musi zadbać, by DoP&C była dostępna przynajmniej w języku kraju przeznaczenia lub w języku angielskim (w zależności od wymagań danego państwa członkowskiego).

---

## Sprzedaż transgraniczna w UE — szczególne obowiązki

Zasada swobodnego przepływu towarów w UE oznacza, że wyrób z oznakowaniem CE może być sprzedawany we wszystkich państwach członkowskich bez dodatkowych krajowych procedur certyfikacyjnych. Niemniej jednak sprzedawca musi pamiętać o:

- **Języku DoP&C** — art. 7 CPR 2024 dopuszcza udostępnienie DoP&C w języku wymaganym przez kraj przeznaczenia. Część państw (np. Niemcy, Francja) wymagają DoP&C w języku krajowym; inne akceptują wersję angielską.
- **Wymogach etykiety** — elementy oznakowania CE na etykiecie lub opakowaniu wyrobu muszą być czytelne i trwałe. Przy sprzedaży online, gdzie fizyczna inspekcja etykiety następuje po dostarczeniu towaru, kluczowe staje się prawidłowe zamieszczenie informacji w listingu.
- **Krajowych organach nadzoru rynku** — każde państwo UE ma własny organ odpowiedzialny za nadzór rynku wyrobów budowlanych. W Polsce jest to Główny Urząd Nadzoru Budowlanego (GUNB). Kontrole rynku online są coraz częstsze.

---

## Co zrobić, gdy wystawiony produkt okaże się niezgodny?

Art. 15 CPR 2024 nakłada na dystrybutora obowiązek działania w przypadku stwierdzenia lub uzasadnionego podejrzenia niezgodności wyrobu:

1. **Niezwłoczne powiadomienie producenta lub importera** o stwierdzonej niezgodności.
2. **Zawieszenie sprzedaży** wyrobu do czasu wyjaśnienia sprawy i podjęcia działań naprawczych.
3. **Powiadomienie organów nadzoru rynku** (GUNB, Inspekcja Handlowa), jeżeli niezgodność stwarza ryzyko dla zdrowia lub bezpieczeństwa.
4. **Przechowywanie dokumentacji** — dystrybutor musi przez 10 lat przechowywać dokumenty potwierdzające przeprowadzenie weryfikacji wyrobu przy jego przyjęciu do oferty.

Zaniechanie tych obowiązków może skutkować solidarną odpowiedzialnością dystrybutora wraz z producentem — w szczególności w przypadku szkód wyrządzonych przez wadliwy wyrób.

---

## Odpowiedzialność sprzedawcy vs producenta — podział ról

Ogólna zasada CPR 2024 jest następująca:

| Obowiązek | Producent | Importer | Dystrybutor |
|---|---|---|---|
| Ocena właściwości (AVCP) | TAK | NIE | NIE |
| Sporządzenie DoP&C | TAK | Może przejąć | NIE |
| Oznakowanie CE | TAK | Może przejąć | NIE |
| Weryfikacja DoP&C i CE | NIE | TAK | TAK |
| Przechowywanie DoP&C (10 lat) | TAK | TAK | TAK |
| Powiadomienie o niezgodności | TAK | TAK | TAK |

Dystrybutor (sklep internetowy) nie jest odpowiedzialny za wystawienie DoP&C ani za badania ITT — ale jest odpowiedzialny za to, że sprzedaje wyłącznie wyroby, dla których te dokumenty istnieją i są dostępne.

---

## Podsumowanie — lista kontrolna dla sklepu internetowego

Przed wystawieniem wyrobu budowlanego w sklepie online warto sprawdzić:

- Czy wyrób posiada wymagane oznakowanie CE (jeśli norma zharmonizowana obowiązuje)?
- Czy DoP&C jest dostępna cyfrowo i aktualnej wersji?
- Czy listing zawiera link do DoP&C lub wskazuje, gdzie ją znaleźć?
- Czy opis produktu jest zgodny z deklarowanymi właściwościami w DoP&C?
- Czy w przypadku sprzedaży transgranicznej DoP&C jest dostępna w wymaganym języku?
- Czy procedury wewnętrzne przewidują reagowanie na informacje o niezgodności wyrobu?

Sprzedaż online wyrobów budowlanych jest wygodna i coraz bardziej popularna — ale wymaga takiej samej staranności prawnej i dokumentacyjnej jak handel tradycyjny.
