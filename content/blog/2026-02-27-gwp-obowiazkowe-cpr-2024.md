---
title: "GWP obowiązkowe od 2027 — jak przygotować dane środowiskowe dla wyrobu budowlanego"
date: "2026-02-27"
author: "dr inż. Katarzyna Lewandowska"
category: "Środowisko"
tags: ["GWP", "ślad węglowy", "EPD", "CPR 2024", "właściwości środowiskowe", "LCA"]
excerpt: "CPR 2024/3110 wprowadza obowiązkowe deklarowanie Globalnego Potencjału Ocieplenia (GWP) dla wyrobów budowlanych. Dowiedz się co to oznacza dla producenta i jak zbierać dane już teraz, zanim termin stanie się pilny."
image_url: "https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?w=800"
template: "analiza"
---

# GWP obowiązkowe od 2027 — jak przygotować się już teraz

Producenci wyrobów budowlanych przez lata mogli ignorować temat śladu węglowego swoich produktów. CPR 2024/3110 kończy ten czas. Globalny Potencjał Ocieplenia (GWP) — potocznie zwany śladem węglowym — stanie się **obowiązkową deklarowaną właściwością** w nowych normach zharmonizowanych, których pierwsze aktualizacje wchodzą w życie od 2027 roku.

Ten artykuł wyjaśnia czym jest GWP, dlaczego staje się kluczowym wskaźnikiem dla branży budowlanej i — przede wszystkim — **co konkretnie musisz zrobić już teraz**, by uniknąć problemów gdy termin stanie się pilny.

## Co to jest GWP i dlaczego właśnie to?

**GWP (Global Warming Potential)** to wskaźnik wyrażający łączny wpływ wyrobu na zmianę klimatu w przeliczeniu na ekwiwalent CO₂ (kg CO₂ eq.). Obejmuje emisje gazów cieplarnianych powstałe podczas:

- wydobycia surowców
- produkcji wyrobu
- transportu
- fazy użytkowania (w przypadku wyrobów energoaktywnych)
- końca życia (utylizacja, recykling)

Budownictwo odpowiada za ok. **40% zużycia energii i 36% emisji CO₂** w UE. Komisja Europejska uznała, że bez deklarowania śladu węglowego materiałów budowlanych niemożliwe jest osiągnięcie celów Europejskiego Zielonego Ładu. Stąd GWP trafia do CPR 2024.

## Co dokładnie mówi CPR 2024/3110?

Rozporządzenie CPR 2024/3110 wprowadza **nowe podstawowe wymaganie nr 8** dotyczące zrównoważonego użytkowania zasobów naturalnych (art. 5 ust. 1 lit. h). Na jego podstawie Komisja Europejska wyda **akty delegowane** wskazujące, jakie właściwości środowiskowe — w tym GWP — mają być deklarowane dla poszczególnych rodzin wyrobów.

Harmonogram wygląda następująco:

| Etap | Termin | Co się dzieje |
|---|---|---|
| Aktualizacje norm zharmonizowanych | 2026–2028 | Normy włączają GWP jako obowiązkową charakterystykę w Załączniku ZA |
| GWP obowiązkowe dla pierwszych rodzin | ok. 2027 | Wyroby z zaktualizowaną normą muszą deklarować GWP w DoP&C |
| Pełna lista właściwości środowiskowych | ok. 2030 | Wszystkie podstawowe wskaźniki środowiskowe |
| Pełne raportowanie cyklu życia | ok. 2032 | Obowiązkowe dane LCA dla wszystkich wyrobów z normą |

Dla producentów kluczowe jest to, że **termin dotyczy daty aktualizacji normy dla danej rodziny wyrobów** — nie jednej globalnej daty. Producent betonu komórkowego może mieć obowiązek wcześniej niż producent uszczelek.

## Które rodziny wyrobów są w pierwszej kolejności?

Na podstawie priorytetów KE i zaawansowania prac normalizacyjnych, najwcześniej zaktualizowane normy spodziewane są dla:

- **Izolacji termicznej** (EN 13162 do EN 13171) — wełna mineralna, styropian, XPS
- **Wyrobów cementowych** — cement, zaprawy
- **Stali konstrukcyjnej** — profile i pręty
- **Prefabrykatów betonowych**
- **Okien i drzwi**

Jeśli Twój wyrób należy do tych rodzin — przygotuj się w pierwszej kolejności.

## Co to jest EPD i czy jest obowiązkowe?

**EPD (Environmental Product Declaration)** — Deklaracja Środowiskowa Produktu — to dokument, który zawiera weryfikowane dane środowiskowe wyrobu, w tym GWP. Sporządza się ją zgodnie z normą **EN 15804+A2**.

Ważne rozróżnienie: EPD jako samodzielny dokument pozostaje **dobrowolna**. Ale **dane środowiskowe** (w tym GWP) zadeklarowane w DoP&C — na podstawie obliczeń LCA zgodnych z EN 15804 — stają się **obowiązkowe** po aktualizacji normy zharmonizowanej.

Innymi słowy: nie musisz publikować pełnego EPD, ale musisz mieć dane, z których możesz wziąć GWP do DoP&C. A te dane i tak wymagają metodyki EN 15804. Efekt praktyczny: posiadanie EPD będzie de facto koniecznością.

## Jak obliczyć GWP dla swojego wyrobu — krok po kroku

### Krok 1: Zbierz dane o surowcach i procesach

Potrzebujesz informacji o:
- liście surowców wejściowych (z ilościami na jednostkę funkcjonalną wyrobu)
- zużyciu energii elektrycznej i cieplnej w produkcji
- transporcie surowców i wyrobów gotowych
- ilości odpadów produkcyjnych

Większość z tych danych powinna być dostępna w Twoim systemie zarządzania produkcją.

### Krok 2: Zamów obliczenia LCA w akredytowanym podmiocie

Obliczenia LCA (Life Cycle Assessment) muszą być zgodne z normą **EN 15804+A2** i **ISO 14044**. Nie są to obliczenia, które można wykonać samodzielnie bez specjalistycznej wiedzy. W Polsce działają firmy oferujące usługi LCA dla wyrobów budowlanych, m.in.:

- instytuty badawcze (ITB, ICiMB)
- wyspecjalizowane firmy konsultingowe ds. zrównoważonego budownictwa
- platformy cyfrowe (One Click LCA, SimaPro z polskim wsparciem)

Koszt obliczeń LCA dla typowego wyrobu budowlanego: **3 000–15 000 PLN** w zależności od złożoności.

### Krok 3: Walidacja przez jednostkę notyfikowaną (opcjonalnie System 3+)

CPR 2024 przewiduje możliwość walidacji danych środowiskowych przez jednostkę notyfikowaną w ramach **Systemu 3+** (nowy system wprowadzony przez CPR 2024). Walidacja nie jest obowiązkowa, ale:
- podnosi wiarygodność deklaracji GWP
- jest wymagana w części przetargów publicznych z kryteriami GPP
- otwiera dostęp do rynków skandynawskich, gdzie EPD są de facto standardem

### Krok 4: Wpisz GWP do DoP&C

Po uzyskaniu obliczeń, wartość GWP (wyrażona w kg CO₂ eq. / jednostkę funkcjonalną) trafia jako jedna z charakterystyk do tabeli właściwości użytkowych w DoP&C.

## Dlaczego warto zacząć teraz, skoro termin jest za rok–dwa?

Trzy powody by nie czekać:

**1. Dane środowiskowe wymagają czasu**
Zebranie danych o łańcuchu dostaw, zlecenie obliczeń LCA i weryfikacja wyników to proces trwający 3–9 miesięcy. Firmy, które zaczną w 2027 gdy termin będzie pilny, będą konkurować o ograniczone moce przerobowe laboratoriów i konsultantów.

**2. Zielone zamówienia publiczne już teraz wymagają EPD**
Część zamówień publicznych — szczególnie w krajach Europy Zachodniej i w Skandynawii, ale coraz częściej też w Polsce — już teraz stawia GWP jako kryterium oceny ofert. Bez EPD nie zakwalifikujesz się do takich przetargów.

**3. Przewaga konkurencyjna**
Producenci, którzy jako pierwsi podadzą GWP, mogą go użyć w marketingu. Niski ślad węglowy to realny argument handlowy wobec inwestorów realizujących budynki BREEAM, LEED lub certyfikowane zgodnie z EPBD.

## Praktyczna checklista dla producenta

- [ ] Sprawdź czy norma zharmonizowana Twojego wyrobu jest w planie aktualizacji KE do 2027
- [ ] Zbierz dane o zużyciu surowców i energii na jednostkę wyrobu
- [ ] Skontaktuj się z dostawcami kluczowych surowców po dane środowiskowe (EPD surowców)
- [ ] Wybierz podmiot do wykonania LCA i zapytaj o termin realizacji
- [ ] Dodaj do DoP&C pole GWP (nawet jeśli na razie NPD — wartość niedeklarowana)
- [ ] Rozważ pełne EPD jeśli celujesz w rynki Europy Zachodniej lub przetargi z kryteriami GPP

## Najczęściej zadawane pytania

**Czy GWP to to samo co „emisje CO₂"?**
GWP obejmuje emisje CO₂ i innych gazów cieplarnianych (metan, podtlenek azotu, freony) przeliczone na ekwiwalent CO₂. Jest to wskaźnik szerszy niż sama emisja CO₂.

**Czy muszę zlecać nowe badania każdorazowo gdy zmienię surowiec?**
Tak, zmiana kluczowego surowca (np. dostawcy cementu w betonie) może istotnie zmienić GWP i wymaga aktualizacji obliczeń LCA oraz DoP&C.

**Gdzie będzie można porównywać GWP różnych wyrobów budowlanych?**
KE planuje europejski rejestr EPD/DPP, który umożliwi porównywanie wyrobów po wskaźnikach środowiskowych. Polska baza EPD jest w fazie tworzenia.

## Podsumowanie

GWP przestaje być dobrowolnym wyróżnieniem — staje się obowiązkiem. Pierwsze normy zharmonizowane z wymaganiem GWP wchodzą w życie ok. 2027, a pełne wdrożenie środowiskowych wymagań CPR 2024 nastąpi do ok. 2032. Producenci, którzy zaczną zbierać dane i obliczać GWP teraz, będą gotowi na termin i zyskają przewagę w zielonych przetargach.

---

**Źródła:**
- Rozporządzenie (UE) 2024/3110 art. 5 ust. 1 lit. h — [EUR-Lex](https://eur-lex.europa.eu/legal-content/PL/TXT/?uri=OJ:L_202403110)
- Norma EN 15804+A2:2019 — podstawa obliczeń LCA dla wyrobów budowlanych
- One Click LCA — [przewodnik po CPR i EPD](https://oneclicklca.com/en/resources/articles/revised-construction-products-regulation-to-include-epd-data)
- Europejski Zielony Ład i budownictwo — [ec.europa.eu](https://ec.europa.eu/info/strategy/priorities-2019-2024/european-green-deal_pl)
